...
public class Main {
    public static void main(String[] args){
        ...
        try {
            ...
            Logger.systemOutLog("ApkMaker error: " + e.getMessage());
            Logger.systemOutLog(Help.help);
            return;
        }
    ...
    public static void main(ApkMakerOptions options){
        main(options, null, null, new Callbacks() {
            ...
            public void updateProgress(String msg, float percent) {
                Logger.systemOutLog(msg + percent + "%");
                System.out.println();
            }

            @Override
            public void error(String str) {
                Logger.systemOutLog(str);
                System.err.println();
            }

            @Override
            public void done(File apk) {
                Logger.systemOutLog("done");
            }
        });
    ...
    public static void main(ApkMakerOptions options, PrintStream out, PrintStream err, Callbacks callbacks){
        ...
        try{
            ...
            Logger.systemOutLog(e.getMessage());
            Logger.systemOutLog(Help.help);
            return;
        }
    }
